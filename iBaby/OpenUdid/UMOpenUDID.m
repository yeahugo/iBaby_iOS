...
+ (NSMutableDictionary*) _getDictFromPasteboard:(id)pboard {
    ...
    // return an shareInstance of a MutableDictionary
...
//    }
    ...
    // know about the two shareInstance methods used on that line; the code running on iOS will work well at run-time.
...
//
+ (NSString*) value {
    return [UMOpenUDID valueWithError:nil];
}
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [UMOpenUDID _getDictFromPasteboard:slotPB];
    ...
    if (openUDID==nil) {        
        if (mostReliableOpenUDID==nil) {
            // this is the case where this app shareInstance is likely to be the first one to use OpenUDID on this device
            // we create the OpenUDID, legacy or semi-random (i.e. most certainly unique)
            //
            openUDID = [UMOpenUDID _generateFreshOpenUDID];
        } else {
    ...
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        if (openUDID && localDict)
            [UMOpenUDID _setDict:localDict forPasteboard:slotPB];
    }
    ...
    if (error!=nil) {
        if (isCompromised)
            *error = [NSError errorWithDomain:kOpenUDIDDomain
                                         code:kUMOpenUDIDErrorCompromised
